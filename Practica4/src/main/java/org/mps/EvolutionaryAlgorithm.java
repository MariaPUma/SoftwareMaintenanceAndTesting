    ...
    public int[][] optimize(int[][] population) throws EvolutionaryAlgorithmException {
        ...
        if (population != null && population.length  > 0 ) {
            ...
            for (int i = 0; i < population.length - 1; i += 2) {
                // AQUÍ HABÍA UN ERROR, INDEXOUTOFBOUNDS. AHORA, CON EL -1, SE OMITE LA ÚLTIMA POBLACIÓN, 
                // ENTONCES NO HAY OUTOFBOUNDS, PERO EL ULTIMO INDIVIDUO NO SE EMPAREJARÁ NUNCA
                
                // Seleccionamos dos individuos de la población actual
                int[] parent1 = selectionOperator.select(population[i]);
                int[] parent2 = selectionOperator.select(population[i + 1]);//Ultimo individuo?
