...
public class RonQI2Silver extends RonQI2{
    ...
    */
    public void obtenerNuevaLectura(){
        ...
        lecturasS.add(disp.leerSensorSonido()); //Error el método correcto es leerSensorSonido()
    ...
    */
    ...
    public boolean evaluarApneaSuenyo() {
        ...
        Double avgP = lecturasP.stream() 
                .mapToDouble(d -> d) //mapea los valores que devuelve a tipo Double
                .average() //les hace la media a esos valores
                .orElse(0.0); //si no hay valores o no se calcula correctamente la media, devuelve 0.0
        Double avgS = lecturasS.stream()
                .mapToDouble(d -> d)
                .average()
                .orElse(0.0);
        
        if (avgP <= thresholdP || avgS <= thresholdS){ // Error: el operador de comparación es <= en ambos casos
            resultado = false;
        }   
        else{
            resultado = true;
